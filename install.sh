...
ark() { "$HOME/core-token/packages/core/bin/run" "$@" --token="woot"; }
echo 'woot() { "$HOME/core-token/packages/core/bin/run" "$@" --token="woot"; }' >> "$HOME/.bashrc"

rm -rf "$HOME/core-token"
git clone "https://github.com/johnwootman/wootchain_core" "$HOME/core-token" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/johnwootman/wootchain_core'"
    exit 1
fi
...
rm -rf "$HOME/.config/@woot"
rm -rf "$HOME/.config/@wootchain"
rm -rf "$HOME/.config/woot-core"
